...
DualNumber<Th, T> quadfuncAD(const DualNumber<Th, T> &x) {
    // get the differentiation of quadratic function: f(x) = x + x^2.
    DualNumber<Th, T> DN(x.Real(), x.Dual());
    DN += x.pow( (size_t)2 );
    return DN;
}
...
{
    vector<BASE_PRECISION> real(3,10);
    vector<BASE_PRECISION> dual(3,10);
    DualNumber<WORKING_PRECISION, BASE_PRECISION> DN(real, dual);
    DualNumber<WORKING_PRECISION, BASE_PRECISION> DN1(3, 10.0);
    ...
    for( auto i = 0; i < 5; ++i )
    {
        cout << i+1 << "   ;    " << (DN_AD.Real())[i] << "    ;    " << (DN_AD.Dual())[i] << endl;
    }
    //
    // for mixed-precision analysis.
